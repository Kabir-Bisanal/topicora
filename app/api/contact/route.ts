...
export async function POST(request: Request) {
  ...
  if (!parsed.success || !submittedAtHumanSpeed(parsed.data.startedAt))
    return Response.json(
      { message: "Check each field and try again." },
      { status: 400 },
    );
  ...
  if (!email.sent)
    return Response.json({
      message:
        "Thanks—we saved your message. Email delivery is not configured, but the editorial inbox can still see it.",
    });
  return Response.json({
    message: "Thanks. Your message was saved and sent to the Topicora team.",
  });
}
