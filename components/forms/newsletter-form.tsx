...
export function NewsletterForm() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch("/api/newsletter", {
      ...
      body: JSON.stringify({
        ...
        consentText: data.get("consent_text"),
        source: "footer",
  ...
  return (
    <form className="relative grid gap-3" onSubmit={submit}>
      <BotFields />
      <label>
      ...
      <label className="text-muted-foreground flex items-start gap-2 text-xs leading-5">
        ...
        <span>
          I agree to receive Topicora’s newsletter and can unsubscribe at any
          time.
        </span>
      </label>
      <input
        type="hidden"
        name="consent_text"
        value="I agree to receive Topicora’s newsletter and can unsubscribe at any time."
      />
