...
const generic =
  "If this address can be subscribed, it will receive the next confirmation step.";
...
export async function POST(request: Request) {
  ...
  if (!parsed.success || !submittedAtHumanSpeed(parsed.data.startedAt))
    return Response.json(
      { message: "Check your email and consent, then try again." },
      { status: 400 },
    );
  ...
  if (!supabase)
    return Response.json(
      {
        message:
          "Newsletter signup is unavailable until the publication database is configured.",
      },
      { status: 503 },
    );
  ...
  if (existing) {
    ...
    if (error && error.code !== "23505")
      return Response.json(
        { message: "Signup could not be saved. Please try again shortly." },
        { status: 503 },
      );
  }
  ...
  if (shouldConfirm) {
    const result = await sendNewsletterConfirmation(
      parsed.data.email,
      `${absoluteUrl("/api/newsletter/confirm")}?token=${encodeURIComponent(token)}`,
    );
    if (!result.sent && result.reason === "not-configured")
      return Response.json({
        message:
          "Your request was saved. Email confirmation is not configured in this local environment.",
      });
  }
