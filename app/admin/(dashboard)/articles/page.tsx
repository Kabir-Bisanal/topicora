...
import { getAdminArticles } from "@/lib/db/admin";
import { formatDate } from "@/lib/utils/date";

export default async function AdminArticlesPage() {
  const [articles, profile] = await Promise.all([
    getAdminArticles(),
    requireStaff("/admin/articles"),
  ]);
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="headline-md mt-2">Articles</h1>
        </div>
        <Link className="button-primary" href="/admin/articles/new">
          <FilePlus2 aria-hidden="true" size={16} /> New article
        </Link>
      </div>
      <div className="border-border bg-surface mt-8 overflow-x-auto rounded-xl border">
          ...
          <tbody className="divide-border divide-y">
            {articles.map((article) => (
              <tr key={article.id}>
                ...
                <td className="p-4 capitalize">{article.status}</td>
                ...
                <td className="p-4">
                  <div className="flex justify-end gap-1">
                    <Link
                      className="button-ghost size-10 p-0"
                    ...
                    {article.status === "archived" ? (
                      <form
                        action={changeArticleStatusAction.bind(
                          null,
                          article.id,
                          "draft",
                        )}
                    ...
                    {profile.role === "admin" ? (
                      <form action={deleteArticleAction.bind(null, article.id)}>
                        <button
                          className="button-ghost text-danger size-10 p-0"
                          type="submit"
        ...
        {articles.length === 0 ? (
          <p className="text-muted-foreground p-8 text-center">
            No articles yet. Create the first draft.
          </p>
        ) : null}
      </div>
    </div>
